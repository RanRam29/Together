# Test Credentials

**IMPORTANT:** These are test accounts that MUST BE DISABLED before going public (production launch).

| Role | Phone | Email | Password | OTP |
|---|---|---|---|---|
| parent | 972524627635 | parent.demo@together.test | admin123 | 123456 |
| professional | 972501111111 | pro.demo@together.test | admin123 | 123456 |
| prof (unverified)* | 972525555555 | pro.unverified@together.test | admin123 | 123456 |
| admin | 972521111111 | admin.demo@together.test | **admin123** | 123456 |
| supervisor | 972522222222 | supervisor.demo@together.test | admin123 | 123456 |
| parent (seed) | 972502222222 | 972502222222@together.test | admin123 | — |

* *prof (unverified)* — חשבון בדיקה. **אין מסמכים מזויפים ב-seed** — יש להעלות מסמכים אמיתיים דרך האפליקציה כ-`pro.unverified@together.test`, ואז לאמת כ-supervisor.

*Note: All seeded users also use `{phone_digits}@together.test` + `admin123` after bulk backfill.*

*Phone+OTP=123456 login requires that the numbers are in the Test OTP list in Supabase Dashboard → Authentication → Phone.*
