...
### 1. שיטות האימות (עודכן — D59, 2026-07-19)

> **שינוי מהמודל המקורי:** עד 2026-07-19 המפרט חייב "טלפון + OTP בלבד, אין סיסמאות". בעל המוצר הכריע לתמוך גם באימייל+סיסמה (עקבי עם עיצוב Stitch שיושם ב-`d281351`). ראו **D59** ב-`product/01-DECISIONS.md`.

- **שלוש שיטות נתמכות** דרך Supabase Auth:
  1. **אימייל + סיסמה** (ברירת המחדל במסך ההתחברות) — `signInWithEmail`/`signUpWithEmail`.
  2. **Google** (OAuth) — כשיוגדר ב-Supabase.
  3. **טלפון + OTP (SMS)** — `sendPhoneOtp`/`verifyPhoneOtp`; נשמר כחלופה ולזרימות פנימיות (D31 — הזמנת הורה שני — נשארת על טלפון+OTP).
- **נימוקי האבטחה המקוריים נגד סיסמאות (reuse/phishing) עדיין תקפים** → חובה: אימות אימייל (`email confirmations` ב-Supabase Auth), מדיניות סיסמה מינימלית, ותהליך איפוס (`requestPasswordReset`/`updatePassword`), ו-rate-limit על login (לא רק על OTP).
- **נורמליזציה:** טלפון נשמר בפורמט E.164 ‏(`+9725...`) בלבד — `lib/phone.ts` הוא המקור היחיד להמרה. אימייל נשמר lowercase-trimmed.
