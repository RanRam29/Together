...
>
> **✅ התחברות חלופית (Email/Password API):** Antigravity סיים את הוספת התמיכה ב-Email Auth ברמת ה-API והמסד. נוספו הפונקציות `signInWithEmail` ו-`signUpWithEmail` ל-`auth-api.ts`, וכן טופל השדה `phone` שיהיה אופציונלי כגיבוי. 
> 
> ⚠️ **משימה ל-Cursor (סוכן ה-UI):** נדרש לעדכן את מסך ההתחברות `login.tsx` כדי לתמוך בבחירה בין "טלפון וקוד" (מנגנון קיים) לבין "אימייל וסיסמה". כמו כן יש לוודא שטופסי ההרשמה דורשים להזין טלפון אם המשתמש נרשם בעזרת אימייל (מועבר דרך ה-`phone` אופציונלי ב-`updateBaseProfile`).
