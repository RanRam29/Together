...
> 🏗️ **ארכיטקט (2026-07-19) — תוכנית הטמעת 41 מסכי Stitch + 2 החלטות + שינוי מודל עבודה.**
> בעל המוצר ייצא מ-Stitch את כל 41 מסכי "Together Design System" (‏`stitch-screens/stitch_together_design_system/` — 39 עם `code.html`, 6 איורים; שמות תיאוריים). נכתבה **תוכנית הטמעה מלאה:** `docs/work-orders/2026-07-19-stitch-screens-implementation-plan.md` (מיפוי כל מסך → S-XXX קנוני → route → סטטוס; רוב המסכים כבר קיימים כ-routes, זה בעיקר re-skin+gap-fill, לא בנייה מאפס).
> **שינוי מודל עבודה (הוראת בעל המוצר):** את גל ה-Stitch **הארכיטקט מבצע ישירות** — לא מועבר ל-Cursor.
> **שתי סתירות של Stitch מול החלטות ליבה — נמצאו וסומנו (דפוס חוזר: Gemini מייצר UI שסותר החלטה שהתקבלה):**
> 1. `messages_inbox` — סותר **D9** (אין צ'אט). **הוכרע: לא נבנה.**
> 2. `search_aides` — סותר מודל ההתאמות המסודרות. **הוכרע: פילטרים על S-PAR-01** → נרשם **D58**.
> 3. `login` (‏`d281351` "match Stitch") — היה **אימייל+סיסמה+Google**, סותר את `AUTH-SPEC` §1 ("טלפון+OTP בלבד"). **הוכרע: לשמור אימייל+סיסמה** → `AUTH-SPEC` §1 עודכן + נרשם **D59** (המודל: אימייל/סיסמה ראשי + Google + טלפון-OTP חלופה; חובה נלווית: אימות אימייל, מדיניות סיסמה, rate-limit על login).
> **G0 (יסודות) — סטטוס:** `tailwind-variants` תקין (hoisted, לא שובר build כפי שחשש הלוח ב-07-16); `tsc --noEmit` **נקי (exit 0)**; טוקני Warm Professionalism כבר ב-`tailwind.config.js`; `login.tsx` כבר תואם Stitch. השער הטכני עבר.
> **פתוח יחיד (לא חוסם):** איחוד `(admin)`↔`(staff)` (D23). **חובה נלווית ל-D59 לאימות מול הקוד:** לוודא `email confirmations` + rate-limit login ב-Supabase (Antigravity, כשנגיע ל-backend).
> ---
> ✅ **ארכיטקט (2026-07-16) — נמצא ותוקן: פער ענק מעל הלוגו בווב (commit `388ae5f`).**
> בעל המוצר צירף הקלטת מסך שהראתה פער ריק ענק (~1600px) מעל הלוגו במסך ההתחברות. אימתתי את הבאג ישירות מול ה-DOM החי (`expo start --web`): `react-native-web` מזריק `height` inline השווה לגובה הפיקסלים הגולמי של קובץ ה-PNG (`require()`-ed, 1856px) כדי לשמור מקום לפני שהתמונה נטענת — וה-inline הזה גובר על `aspectRatio` (שממלא רק ממד שלא הוגדר), כך שהקופסה קרסה לגובה 1856px המלא של קובץ המקור עם הלוגו מצויר זעיר בתוכה. זו הייתה תוצאה ישירה של הקומיט `9d939ab` (התיקון הקודם ל"יחס הגובה-רוחב של הלוגו") שעבר מ-width+height מפורשים ל-`aspectRatio`+ממד יחיד — עבד תקין בנייטיב, נשבר בווב. **תוקן** ב-`AppLogo.tsx`, `SplashReveal.tsx`, `EmptyState.tsx`: חזרה ל-width+height מפורשים (מחושבים מהיחס הידוע 2266:1856) בלי להסתמך על `aspectRatio` בכלל. אומת ישירות: קופסת הלוגו כעת 240×197px במקום 240×1856px.
> **תצפית נוספת (לא טופלה, לא בהיקף):** בזמן העבודה מצאתי 4 קבצים חדשים לא-מקומיטים (`components/ui/Avatar.tsx`, `Badge.tsx`, `Button.tsx`, `Card.tsx`) שתלויים בחבילה `tailwind-variants` שלא מותקנת — שוברים את `tsc --noEmit`. אלה לא שלי ולא נגעתי בהם (נראה כמו עבודה חיה של Cursor באותו working tree). **מי שעובד על זה: תזכרו להתקין `tailwind-variants` לפני commit, אחרת ה-CI ייכשל.**
